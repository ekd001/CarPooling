...
class MainActivity : AppCompatActivity(), OnMapReadyCallback {
        ...
        bottomNavigation.setOnItemSelectedListener { item ->
            when (item.itemId) {
                ...
                R.id.nav_profile -> {
                    ...
                    val username = intent.getStringExtra(Constants.TAG_USER_USERNAME)
                    val email = intent.getStringExtra(Constants.TAG_USER_EMAIL)
                    val carLicenseNumber = intent.getStringExtra(Constants.TAG_USER_CAR_LICENSE_NUMBER)
                    Log.i("MainActivity", "Username: $username")
                    var bundleProfile = Bundle()
                    bundleProfile.putString("id", intent.getStringExtra(Constants.TAG_USER_ID))
                    bundleProfile.putString("username", intent.getStringExtra(Constants.TAG_USER_USERNAME))
                    bundleProfile.putString("email", intent.getStringExtra(Constants.TAG_USER_EMAIL))
                    bundleProfile.putString("carLicenseNumber", intent.getStringExtra(Constants.TAG_USER_CAR_LICENSE_NUMBER))
    ...
    private fun updateMapWithLocation(location: LatLng) {
        ...
        googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(location, 25f))
    }
